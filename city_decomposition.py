...
class SubCity:
    def __init__(self, h, w, residental_projects, utility_projects, distance):
        self.w = w
        self.h = h
...
class City:
    ...
    def construct_scenario(self):
        ...
        while not finished:
            ...
            if residental_projects:
                residental_project = residental_projects.pop()
    ...
    def unit_city(self, _list):
        ...
        for sub_city in _list:
            ...
            self.construct_building(start_point, sub_city)

            for residental_project in sub_city.residental_building:
                updated_cell = []
                for x, y in residental_project.coord:
                    _x, _y = (x + start_point[0], y + start_point[1])
                    updated_cell.append((_x, _y))


                residental_project.coord = updated_cell

                city_plan.residental_building.append(residental_project)

            for utility_project in sub_city.utility_building:
                updated_cell = []
                for x, y in utility_project.coord:
                    _x, _y = (x + start_point[0], y + start_point[1])
                    updated_cell.append((_x, _y))

                utility_project.coord = updated_cell

                city_plan.utility_building.append(utility_project)

            city_plan.plan = self.plan

            if start_point[1] + sub_city.w * 2 <= self.w:
                start_point = (start_point[0], start_point[1] + sub_city.w)
            else:
                start_point = (start_point[0] + sub_city.h, 0)
    ...
    def construct(self):
        ...
        for i in range(count):
            ...
            sub_city = SubCity(sub_city_h, sub_city_w, residental_projects=[residental_projects.pop()],
                               utility_projects=[utility_projects.pop()], distance=self.distance)
    ...
    def construct_building(self, start_point, project):
        for x in range(len(project.plan)):
            for y in range(len(project.plan[0])):
                _x, _y = (x + start_point[0], y + start_point[1])
                self.plan[_x][_y] = project.plan[x][y]
...
if __name__ == '__main__':
    ...
    parser = ParseInput()
    city = parser('d_wide_selection.in')
